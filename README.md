...
---
### 📕Документация
Все доступные API методы и их работа должны быть доступны тут -> [`документация`](http://127.0.0.1:8000/api/docs/), после старта приложения в докер контейнере.
...
Фильтр для свободных комнат уже написан для предыдущего задания и в этом коде будет просто переиспользоваться,
то стоит вопрос "Как обеспечить атомарность операции бронирования?" -простой прогон по фильтру и дальнейшая запись не лучши вариант, так как между 
